...
chunk存储
*/
func PieceSave(file multipart.File, pieceMd5 string, fileMd5 string) int64 {
	//创建上传目录
	path := "./uploadFile/chunks/" + fileMd5 + "/"
...
文件合并
*/
func (fd *FileDealer) MergeFile(fileMd5, filename string, chunks []string, cnt int, lastModified int64) (string, string, string) {
	...
	itemPath := ""
	for i := 0; i < cnt; i++ {
		itemPath = "./uploadFile/chunks/" + fileMd5 + "/" + chunks[i]
